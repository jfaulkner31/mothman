...
def _rotate_point_45(point: tuple[float, float, float], origin: tuple[float, float, float]) -> tuple[float, float, float]:
  """
...
def _make_paraview_rect_prism(x: tuple, y: tuple, z: tuple, origin: tuple[float, float, float]) -> str:
  """
  ...
  return [
    _rotate_point_45((lx, ly, lz), origin),
    _rotate_point_45((ux, ly, lz), origin),
    _rotate_point_45((ux, uy, lz), origin),
    _rotate_point_45((lx, uy, lz), origin),
    _rotate_point_45((lx, ly, uz), origin),
    _rotate_point_45((ux, ly, uz), origin),
    _rotate_point_45((ux, uy, uz), origin),
    _rotate_point_45((lx, uy, uz), origin),
  ]

  return the_rect_prism_str

def dump_ch_array_to_paraview(arr: ChannelArray, dx: float, dy: float, outname: str, time: float = 0.0):
  """
  arr : ChannelArray
  ...
  time : float
    The simulation time for this data dump.
  """
  ...
  for idx, ch in enumerate(arr.channels):
    ...
    for cid in ch.mesh.cells.keys():
      ...
      local_pts = _make_paraview_rect_prism((lowerX, upperX), (lowerY, upperY), (lowerZ, upperZ), origin)
  ...
  for name in fields_dict.keys():
    ...
    farr.text = " ".join(f"{v:.6e}" for v in fields_dict[name])

  # Field data for time
  fielddata = ET.SubElement(vtk, "FieldData")
  time_array = ET.SubElement(fielddata, "DataArray",
                             type="Float64", Name="TimeValue", NumberOfTuples="1", format="ascii")
  time_array.text = str(time)
